...
export async function POST(req: NextRequest) {
    try {
        ...
        booking.dropoffOtp = otp;
        booking.dropoffOtpExpires = new Date(Date.now() + 5 * 60 * 1000)
        await booking.save();

        const emailTemplate = `
      <div style="font-family: DM-sans, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2>Ride OTP</h2>
        <p>Please share this following One-Time Password (OTP) to your driver to complete the ride. This code is valid for 5 minutes.</p>
    ...
    `;
        ...
        if (booking.user.email) {
            await sendMail(booking.user.email, "Your Dropoff OTP - GLIDE", emailTemplate)
        }

        return NextResponse.json(
            { message: "Dropoff OTP sent/generated successfully." },
            { status: 200 }
        );

    } catch (error) {
        console.log("Error Sending Dropoff OTP", error);
        return NextResponse.json(
